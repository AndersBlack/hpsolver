...
pub fn update_passing_precondition_total(called: &(Vec<bool>, Vec<(Method, RelVars, Vec<Precondition>)>, Vec<usize>), passing_preconditions: &Vec<Precondition>, parameters: &Vec<Argument>) -> Vec<Precondition> {
	let mut new_passing_precon = Vec::<Precondition>::new();
