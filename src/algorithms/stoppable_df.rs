		...
		match current_node {
			Some(mut current_node) => {
				...
				if state_exists {
					...
					if !algorithms::method_calls_method(&domain.methods) {
						continue;
					}
				}
				...
				match current_subtask {
					...
					Some((SubtaskTypes::Action(action), relevant_variables)) => {
						...
						algorithms::perform_action(node_queue, current_node, action, relevant_variables);
					},
					None => { 
...
// 		// We have finished with this methods subtasks 
...
// 			// Adding the action to the list of applied actions
