...
use crate::{datastructures::{node::*, domain::*, problem::{*}}, toolbox::reduce_domain};
use crate::toolbox::{self};

// Relevant Variables datatype
type RelVars = Vec<(String, String, Vec<String>)>;

pub fn stoppable_depth_first(problem: Problem, domain: &Domain, stopped: &Instant) -> &'static str {
	...
	let return_string = run_df(&mut node_queue, &new_domain, stopped);

  return_string
}

fn run_df(node_queue: &mut Vec::<Node>, domain: &Domain, stopped: &Instant) -> &'static str {
